...
class DashboardViewModel @Inject constructor(): ViewModel() {
    ...
    @Inject lateinit var dashBoardUseCase: IDashBoardUseCase
    ...
    fun getRemoteDate() {
        viewModelScope.launch {
            withContext(Dispatchers.IO) {
               dashBoardUseCase.getDataRemote(10.838741, 106.802248,"710cdcfa5ef43deff6842b2eb49a147e").onSuccess {
    ...
    fun getCareEmployeeList() {
        viewModelScope.launch {
            withContext(Dispatchers.IO) {
                dashBoardUseCase.getCareEmployees().onSuccess {
    ...
    fun getBannerList() {
        viewModelScope.launch {
            withContext(Dispatchers.IO) {
                dashBoardUseCase.getBannerList().onSuccess {
